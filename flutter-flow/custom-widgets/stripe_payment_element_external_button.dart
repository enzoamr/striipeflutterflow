...
/// Custom Widget: StripePaymentElement (Version avec bouton externe)
///
/// NOUVELLE VERSION: Permet d'utiliser un bouton Flutter Flow externe
///
/// Paramètres:
/// - stripePublishableKey: Votre clé publique Stripe (pk_test_... ou pk_live_...)
/// - clientSecret: Le client secret du Payment Intent
/// - hideButton: Si true, cache le bouton intégré (utilisez un bouton externe)
/// - onPaymentSuccess: Callback appelé quand le paiement réussit
/// - onPaymentError: Callback appelé en cas d'erreur
/// - onReadyToProcess: Callback appelé quand le widget est prêt (renvoie une fonction pour déclencher le paiement)
///
/// UTILISATION AVEC BOUTON EXTERNE:
/// 1. Mettez hideButton = true
/// 2. Utilisez onReadyToProcess pour récupérer la fonction de paiement
/// 3. Stockez cette fonction dans App State
/// 4. Appelez-la depuis votre bouton Flutter Flow via une Custom Action
class StripePaymentElement extends StatefulWidget {
  ...
  final Future<dynamic> Function(Future<void> Function())? onReadyToProcess;  // NOUVEAU: Callback qui passe la fonction de paiement
  final String buttonText;
  final Color buttonColor;
  final bool hideButton;  // NOUVEAU: Permet de cacher le bouton
...
class _StripePaymentElementState extends State<StripePaymentElement> {
  ...
  void _initializeStripeElement(html.Element container) {
    ...
    // Marquer comme prêt et notifier le parent si callback fourni
    setState(() {
      _isReady = true;
    });

    // NOUVEAU: Si onReadyToProcess est fourni, passer la fonction _handlePayment
    if (widget.onReadyToProcess != null) {
      widget.onReadyToProcess!(_handlePayment);
    }
  }
