...
/// Custom Widget: StripePaymentElement avec JavaScript Bridge
///
/// VERSION AVANCÉE: Permet d'utiliser un bouton Flutter Flow externe
///
/// Ce widget affiche le Stripe Payment Element et expose une fonction JavaScript
/// permettant de déclencher le paiement depuis n'importe quel bouton Flutter Flow
///
/// Paramètres:
/// - stripePublishableKey: Votre clé publique Stripe (pk_test_... ou pk_live_...)
/// - clientSecret: Le client secret du Payment Intent
/// - hideButton: Si true, cache le bouton intégré
/// - buttonText: Texte du bouton intégré (si visible)
/// - buttonColor: Couleur du bouton intégré (si visible)
/// - onPaymentSuccess: Callback appelé quand le paiement réussit
/// - onPaymentError: Callback appelé en cas d'erreur
///
/// UTILISATION AVEC BOUTON EXTERNE:
/// 1. Mettez hideButton = true
/// 2. Créez votre bouton Flutter Flow où vous voulez sur la page
/// 3. Dans l'action du bouton, appelez la Custom Action: triggerStripePayment
/// 4. Le paiement se déclenchera automatiquement
///
/// EXEMPLE:
...
///
/// Bouton Flutter Flow:
///   Texte: "Payer maintenant"
///   Actions:
///     1. Backend Call → triggerStripePayment
///     2. (le reste de vos actions success/error est géré par les callbacks du widget)
...
class _StripePaymentElementState extends State<StripePaymentElement> {
  ...
  void _initializeStripeElement(html.Element container) {
    ...
    // Ceci permet d'appeler _handlePayment depuis n'importe quel bouton Flutter Flow
    js.context['triggerStripePayment'] = js.allowInterop(() {
      _handlePayment();
    });

    print('✅ Stripe Payment Element initialisé. Fonction JavaScript exposée.');
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Bouton de paiement (visible seulement si hideButton = false)
          ...
          // Indicateur de traitement (si bouton caché)
